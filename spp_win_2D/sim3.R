...
load(here("GlacierBay_Code","spp_win_2D", "sim3.RData"))
set.seed(1234)
...
# simulate points w/ Neyman-Scott dependence
lambda.fun <- function(x, y) {
  beta.0 + beta[1]*x + beta[2]*y
}

survey.win <- owin(xrange = x.domain, yrange = y.domain)
parent.candidates <- rpoispp(lam.max, win = survey.win)
parent.lambda.idx <- cellFromXY(full.raster, cbind(x = parent.candidates$x,
                                                   y = parent.candidates$y))
parent.lambda <- values(full.raster)[parent.lambda.idx,3]

obs.idx <- rbinom(parent.candidates$n, 1, parent.lambda/lam.max) == 1
parents <- parent.candidates[obs.idx]
...
for (i in 1:parents$n) {
  ...
  n.children <- rpois(1, lambda = 2*l)
  
  if (n.children > 0) {
    dx <- rnorm(n.children, mean = 0, sd = sigma)
    dy <- rnorm(n.children, mean = 0, sd = sigma)
    x <- x0 + dx
    y <- y0 + dy
    offspring[[i]] <- ppp(x, y, window = survey.win)
  }
}

all.offspring <- do.call(superimpose, c(offspring, list(W = survey.win)))
all.points <- superimpose(parents, all.offspring)

plot(all.points)
s.obs <- cbind(x = all.points$x, y = all.points$y)
X.obs <- s.obs
N <- all.points$n
...
ggplot(data = obs.df, aes(x = all.points$x, y = all.points$y,
                          col = factor(obs.win))) +
  geom_point()
...
# --- Non-Bayesian GLM w/ ELM (exact, logistic) --------------------------------
## stage 1
X.full.aug <- rbind(X.full, X.bg)

source(here("GlacierBay_Code", "spp.logit.ELM.R"))
tic()
out.glm <- spp.logit.ELM(X.bern, X.full.aug, y.bern, n, p)
toc()

beta.glm <- out.glm$beta.glm
vcov.glm <- out.glm$vcov.glm
# sample from glm estimated density
beta.save <- t(mvnfast::rmvn(n.mcmc, mu = beta.glm, sigma = vcov.glm))
W.full <- out.glm$W.full
W.obs <- out.glm$W.obs
W.win.full <- W.full[full.win.idx,]
