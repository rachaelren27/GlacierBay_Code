...
calc_DIC <- function(D,beta.post,X.obs,X.full,ds,n){
...
# set design matrices
X.win.full.list[[1]] <- as.matrix(X.win.full[,1])
X.obs.list[[1]] <- as.matrix(X.obs[,1])

X.win.full.list[[2]] <- as.matrix(X.win.full[,2])
X.obs.list[[2]] <- as.matrix(X.obs[,2])

X.win.full.list[[3]] <- as.matrix(X.win.full[,3])
X.obs.list[[3]] <- as.matrix(X.obs[,3])

X.win.full.list[[4]] <- X.win.full[,1:2]
X.obs.list[[4]] <- X.obs[,1:2]

X.win.full.list[[5]] <- X.win.full[,c(1,3)]
X.obs.list[[5]] <- X.obs[,c(1,3)]

X.win.full.list[[6]] <- X.win.full[,2:3]
...
beta.post.list <- foreach(k = 1:n.models) %dopar% {
  spp.comp.mcmc(seal.mat, X.win.full.list[[k]], X.obs.list[[k]],
                                  ds, n.mcmc, 0.1, 0.1)
}
...
for(i in 1:n.models){
  ...
  DIC[[i]] <- calc_DIC(D, beta.post, X.win.full.list[[i]], X.obs.list[[i]], ds, n)
}
...
m7.DIC <- DIC(beta.post, X.obs, X.win.full, ds, n)
