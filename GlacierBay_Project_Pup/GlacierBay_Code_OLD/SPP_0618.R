...
n.mcmc=100000
source(here("GlacierBay_Code_OLD", "spp_win_2D", "spp.comp.mcmc.gamma.R"))
tic()
...
bath.full <- cbind(values(bath.rast.survey), row.counts)
bath <- na.omit(bath.full)
X.bg <- cbind(bath, glac.dist)
bg.idx <- c()
for(i in 1:nrow(X.full)){
  if(X.bg[i,2] != 0){
    bg.idx <- c(bg.idx, rep(i, times = X.bg[i,2]))
  }
}
...
X.obs.bg <- X.bg[c(seal.idx, bg.idx),] 
...
plot(x = beta.save.full.lik[2,], y = exp(beta.0.save.full.lik))
lines(x = beta.save[2,], y = exp(beta.0.save), col = 2)
