...
full.df <- as.data.frame(cbind(s.full, x1 = (x-0.5)^2 + (y-0.5)^2, x2 = x + y))

x1.raster <- rasterFromXYZ(as.data.frame(cbind(s.full, z = (x-0.5)^2 + (y-0.5)^2)))
x2.raster <- rasterFromXYZ(as.data.frame(cbind(s.full, z = x + y)))
X.full <- scale(as.matrix(full.df[,-(1:2)]))
...
lam.df <- as.data.frame(cbind(s.full, z = lam.full))
lam.raster <- rasterFromXYZ(lam.df)
plot(lam.raster)
...
s.superpop <- cbind(x = x.superpop, y = y.superpop)
X.superpop <- cbind((x.superpop - 0.5)^2 + (y.superpop - 0.5)^2,
                    x.superpop + y.superpop)
...
obs.raster.idx <- cellFromXY(x1.raster, xy = s.obs)
X.obs <- cbind(values(x1.raster)[obs.raster.idx], values(x2.raster)[obs.raster.idx])
lam.obs <- lam.superpop[obs.idx]
N=nrow(s.obs) # 218

obs.df <- as.data.frame(s.obs, lam.obs)
ggplot(data = obs.df, aes(x = x, y = y, col = lam.obs)) + 
  geom_point()
...
lam.win.obs <- exp(beta.0+X.win%*%beta)
obs.win.df <- as.data.frame(cbind(s.win, lam.win.obs))
ggplot(data = obs.win.df, aes(x = x, y = y, col = V3)) + 
  geom_point()
...
n.mcmc=100000
source(here("GlacierBay_Code", "s", "spp.cond.mcmc.R"))
...
# prepare X matrix
bg.idx <- cellFromXY(x1.raster, xy = bg.pts)
X.bg <- cbind(values(x1.raster)[bg.idx], values(x2.raster)[bg.idx])
...
source(here("spp_win_2D", "Polya_Gamma.R"))
